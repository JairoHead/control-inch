...
<div class="py-8 px-4 sm:px-6 lg:px-8 flex justify-center">
    <div class="w-full max-w-2xl"> {{-- Limita el ancho del formulario --}}
        ...
        <div class="bg-white dark:bg-gray-800 shadow-xl rounded-lg overflow-hidden">
            <div class="p-6 sm:p-8">
                ...
                <form action="{{ route('inspectores.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                        ...
                        <div class="md:col-span-2">
                            <label class="block text-gray-700 dark:text-gray-300 text-sm font-bold mb-2" for="dni">
                                DNI <span class="text-xs font-light text-gray-500 dark:text-gray-400">(Opcional, 8 caracteres)</span> {{-- Quitamos asterisco, añadimos nota --}}
                            </label>
                            <input class="form-input @error('dni') input-error @enderror w-full"
                                   id="dni" type="text" name="dni" value="{{ old('dni') }}"
                                   maxlength="8" {{-- Limitador HTML --}}
                                   pattern="\d{8}" {{-- Opcional: patrón HTML para solo números --}}
                                   title="El DNI debe contener 8 dígitos (opcional)."> {{-- Tooltip --}}
                                   {{-- Quitamos el atributo 'required' --}}
