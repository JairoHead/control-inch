    ...
    <div class="bg-white dark:bg-gray-800 shadow-xl rounded-lg overflow-hidden">
        ...
        <div class="px-6 py-4 sm:px-8 sm:py-4"> 
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center mb-4 gap-4">
                <h1 class="text-2xl sm:text-3xl font-bold text-black-900 dark:text-gray-100">
                    Lista de Inspectores
                </h1>
                {{-- Botón normal que redirige a la ruta create --}}
                <a href="{{ route('inspectores.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 active:bg-blue-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150 flex-shrink-0"> {{-- Restaturado estilo completo --}}
            ...
            <div class="mb-4">
                <label for="inspectores-search" class="sr-only">Buscar Inspectores</label> {{-- Cambiado ID para evitar colisión --}}
                <input type="search" id="inspectores-search"
                       wire:model.live.debounce.300ms="searchTerm"
                       placeholder="Buscar por DNI, Nombre o Apellido..."
                       class="form-input w-full"> {{-- Asumiendo clase CSS --}}
            </div>
            ...
            <div class="overflow-x-auto mt-4">
                <table class="min-w-full divide-y divide-gray-300 dark:divide-gray-700">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-800">
                        ...
                        @forelse ($inspectores as $inspector)
                            <tr class="odd:bg-gray-200 dark:odd:bg-gray-700 even:bg-white dark:even:bg-gray-800 hover:bg-blue-100 dark:hover:bg-blue-900/30" wire:key="inspector-{{ $inspector->id }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                    ...
                                    <a href="{{ route('inspectores.edit', $inspector) }}" class="inline-flex items-center text-amber-600 hover:text-amber-800 dark:text-amber-400 dark:hover:text-amber-300" title="Editar Inspector">
                                    ...
                                    <form action="{{ route('inspectores.destroy', $inspector) }}" method="POST" class="inline" onsubmit="return confirm('¿Estás seguro de dar de baja a este inspector?');">
                                        @csrf @method('DELETE')
                        ...
                        @empty
                            {{-- Mensaje vacío --}}
                            <tr wire:loading.remove> {{-- Ocultar si está cargando --}}
                                <td colspan="3" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center text-gray-500 dark:text-gray-400">
                                        ...
                                        @if(strlen($searchTerm) > 0)
                                            <p>No se encontraron inspectores que coincidan con "{{ $searchTerm }}".</p>
                                        @else
